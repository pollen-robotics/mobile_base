...
class ZuuuGotoActionServer(Node):
            ...
            if self.nb_remaining_deletions == 0:
                ...
                if ret == "no_longer_goto_mode":
                    ...
                    self.get_logger().warning(
                        f"A mode change happened during the goto execution. Removing all ({self.nb_remaining_deletions}) goals from the queue."
                    )
    ...
    def setup_goto(self, goto_request):
        ...
        self.zuuu_hal.distance_pid = PID(
            ...
            i=goto_request.distance_i,
            d=goto_request.distance_d,
            max_command=goto_request.distance_max_command,
            max_i_contribution=goto_request.distance_max_command / 2.0,
        )
        self.zuuu_hal.angle_pid = PID(
            p=goto_request.angle_p,
            i=goto_request.angle_i,
            d=goto_request.angle_d,
            max_command=goto_request.angle_max_command,
            max_i_contribution=goto_request.angle_max_command / 2.0,
        )
