...
def generate_launch_description():
    ...
    rviz_config_dir = os.path.join(get_package_share_directory("rplidar_ros2"), "rviz", "rplidar_ros2.rviz")

    return LaunchDescription(
        [
            DeclareLaunchArgument("channel_type", default_value=channel_type, description="Specifying channel type of lidar"),
            DeclareLaunchArgument("tcp_ip", default_value=tcp_ip, description="Specifying tcp ip to connected lidar"),
            DeclareLaunchArgument("tcp_port", default_value=tcp_port, description="Specifying tcp port to connected lidar"),
            DeclareLaunchArgument("frame_id", default_value=frame_id, description="Specifying frame_id of lidar"),
            DeclareLaunchArgument(
                "inverted", default_value=inverted, description="Specifying whether or not to invert scan data"
            ),
            DeclareLaunchArgument(
                "angle_compensate",
